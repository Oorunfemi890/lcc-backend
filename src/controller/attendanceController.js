...
const attendanceController = {
  ...
  createAttendance: async (req, res) => {
    try {
      ...
      logger.info('Attendance record created:', newAttendance.id);

      // ✅ FIXED: Create member attendance records - timeArrived is optional
      if (members && Array.isArray(members) && members.length > 0) {
        const memberAttendanceRecords = members
          .filter((member) => {
            // Must have memberId and present must be true
            return member.memberId && (member.present === true || member.present === 'true');
          })
          .map((member) => ({
            attendanceId: newAttendance.id,
            memberId: member.memberId,
            present: true,
            timeArrived: member.timeArrived || null, // ✅ Optional - can be null
            notes: member.notes || null, // ✅ Optional - can be null
          }));

        if (memberAttendanceRecords.length > 0) {
          try {
            await MemberAttendance.bulkCreate(memberAttendanceRecords);
            logger.info(`✅ Created ${memberAttendanceRecords.length} member attendance records`);
          } catch (memberError) {
            logger.error('Error creating member attendance records:', memberError);
            // Don't fail the entire request, just log the error
            logger.warn('Attendance created but member tracking failed');
          }
        }
      }
  ...
  updateAttendance: async (req, res) => {
    try {
      ...
      if (members && Array.isArray(members) && members.length > 0) {
        ...
        const memberAttendanceRecords = members
          .filter((member) => member.present === true || member.present === 'true')
          .map((member) => ({
